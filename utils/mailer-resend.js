...
/**
 * Sends a password reset email to the user using Resend API.
 * @param {string} email - The recipient's email address.
 * @param {string} code - The 6-digit reset code.
 */
async function sendPasswordResetEmail(email, code) {
  if (!resend) {
    console.log(`[DEV MODE] Email suppression: Password reset code for ${email} is ${code}`);
    return true;
  }
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: process.env.EMAIL_FROM || 'PhysioSim <onboarding@resend.dev>',
      to: [email],
      subject: "Reset your PhysioSim password",
      html: `
            ...
            <td align="center">
                ...
                <table width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width: 600px; background-color: #ffffff; border-radius: 20px; box-shadow: 0 10px 25px rgba(0, 0, 0, 0.05); overflow: hidden;">
                    ...
                    <tr>
                        <td style="padding: 50px 40px;">
                            ...
                            <!-- Instructions -->
                            <p style="color: #64748b; font-size: 15px; line-height: 1.6; text-align: center; margin: 0 0 40px 0;">
                                Enter this 6-digit code on the password reset page to verify your identity and set a new password.
                            </p>
                    ...
                    <tr>
                        <td style="background-color: #f8fafc; padding: 30px 40px; border-top: 1px solid #e2e8f0;">
                            ...
                            <!-- Security Note -->
                            <p style="color: #94a3b8; font-size: 12px; line-height: 1.5; text-align: center; margin: 0 0 20px 0;">
                                <strong>⚠️ Security Warning:</strong> If you did not request a password reset, please ignore this email. Your password will remain unchanged.
                            </p>
                ...
                <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-top: 30px;">
                    <tr>
                        <td align="center">
                            <p style="color: #94a3b8; font-size: 12px; text-align: center;">
                                This email was sent because a password reset was requested for your PhysioSim account.
                            </p>
                        </td>
                    </tr>
                </table>
